...
flockApp.controller('searchPageController', function ($scope, $location, $window, userService, quackService, sessionFactory) {
    ...
    self.hashtag = self.location.substr(self.location.indexOf("hashtag=") + ("hashtag=".length), getParamLength(self.location, "hashtag="));
    $scope.searchQuery = self.hashtag;
    console.log(self.hashtag);
    ...
    $scope.refreshQuacks = function () {
        if (!($scope.replyMode)) {
            ...
            quackService.getAllQuacksByHashtag($scope.searchQuery).then(function (data) {
